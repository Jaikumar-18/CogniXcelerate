...
import ActionButton from '@/views/canvas/ActionButton'
...
const AgentflowCanvas = () => {
    ...
    const onConnect = (params) => {
        ...
        // Always use the header UI color for consistency
        const targetColor = '#3949ab'
        const sourceColor = '#3949ab'
    ...
    const handleAddNode = () => {
        // Trigger the AddNodes component's toggle functionality
        if (addNodesRef.current) {
            addNodesRef.current.handleToggle()
        }
    }

    const handleEmbed = () => {
        // Trigger the CanvasHeader's embed functionality
        if (canvasHeaderRef.current) {
            canvasHeaderRef.current.onAPIDialogClick()
        }
    }

    const handleSettings = () => {
        // Trigger the CanvasHeader's settings functionality
        if (canvasHeaderRef.current) {
            canvasHeaderRef.current.setSettingsOpen(true)
        }
    }
    ...
    const handleSave = () => {
        if (chatflow?.id) {
            ...
            // Open save dialog through CanvasHeader
            if (canvasHeaderRef.current) {
                canvasHeaderRef.current.setFlowDialogOpen(true)
            }
        }
    }

    const handleGenerateAgentflow = () => {
        // Trigger the AddNodes component's generate functionality
        if (addNodesRef.current) {
            addNodesRef.current.handleOpenDialog()
        }
    }
                        ...
                        <div className='reactflow-wrapper' ref={reactFlowWrapper}>
                            ...
                            >
                                ...
                                <ActionButton
                                    ...
                                    onSave={handleSave}
                                    onGenerateAgentflow={handleGenerateAgentflow}
                                    isAgentCanvas={true}
                                    savePermission='agentflows:create'
                                    isDirty={canvas.isDirty}
                                    nodesData={getNodesApi.data}
                                />
                                {/* Hidden AddNodes component for functionality */}
                                <AddNodes
                                    ref={addNodesRef}
                                    isAgentCanvas={true}
                                    nodesData={getNodesApi.data}
