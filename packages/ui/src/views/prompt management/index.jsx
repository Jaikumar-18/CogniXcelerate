...
import { IconTrash, IconPlus, IconCrown, IconCrownOff, IconMessage, IconSearch } from '@tabler/icons-react'
...
const PromptManagement = () => {
    ...
    // Single source of truth for the Langfuse endpoint used in prompt management
    const BASE_URL = 'https://xcelerate.cogniwide.com:6443/langfuse'
    const API_PREFIX = `${BASE_URL}/api/public/v2`
    ...
    const fetchPrompts = async (pageToFetch = page) => {
        try {
            const response = await axios.get(`${API_PREFIX}/prompts?page=${pageToFetch}&limit=${limit}`, {
                headers: {
    ...
    const handleVersionClick = async (prompt) => {
        try {
            ...
            try {
                const baseResponse = await axios.get(`${API_PREFIX}/prompts/${prompt.name}`, {
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization:
                            'Basic cGStbGYtNWEyYmFlZmMtZTJjZi00ODBkLTlhNGQtNDY4NjAwYTZiNmE0OnNrLWxmLWJkMzk2ZTViLWE0MzgtNGRlYi04YzgxLTFjOTU4MTNjNGQ3OQ=='
                ...
                for (let version of prompt.versions) {
                    try {
                        ...
                        const versionResponse = await axios.get(
                            `${API_PREFIX}/prompts/${prompt.name}?version=${version}`,
                            {
                                headers: {
                                    'Content-Type': 'application/json',
                                    Authorization:
    ...
    const setVersionAsProduction = async (promptName, versionNumber) => {
        try {
                ...
                if (currentPrompt && currentPrompt.versions && Array.isArray(currentPrompt.versions)) {
                    ...
                    for (const versionNum of currentPrompt.versions) {
                        try {
                            ...
                            const versionResponse = await axios.get(
                                `${API_PREFIX}/prompts/${promptName}?version=${versionNum}`,
                                {
                                    headers: {
                                        'Content-Type': 'application/json',
                                        Authorization:
                    ...
                    const response = await axios.get(`${API_PREFIX}/prompts/${promptName}`, {
                        headers: {
                            'Content-Type': 'application/json',
                            Authorization:
                                'Basic cGStbGYtNWEyYmFlZmMtZTJjZi00ODBkLTlhNGQtNDY4NjAwYTZiNmE0OnNrLWxmLWJkMzk2ZTViLWE0MzgtNGRlYi04YzgxLTFjOTU4MTNjNGQ3OQ=='
            ...
            for (const version of allVersions) {
                if (version.labels?.includes('production')) {
                    ...
                    try {
                        ...
                        await axios.patch(
                            `${API_PREFIX}/prompts/${promptName}/versions/${version.versionNumber}`,
                            {
            ...
            const response = await axios.patch(
                `${API_PREFIX}/prompts/${promptName}/versions/${versionNumber}`,
                {
                ...
                if (currentPrompt && currentPrompt.versions && Array.isArray(currentPrompt.versions)) {
                    ...
                    for (const versionNumber of currentPrompt.versions) {
                        try {
                            ...
                            const versionResponse = await axios.get(
                                `${API_PREFIX}/prompts/${promptName}?version=${versionNumber}`,
                                {
                                    headers: {
                                        'Content-Type': 'application/json',
                                        Authorization:
                ...
                try {
                    ...
                    try {
                        const verifyResponse = await axios.get(
                            `${API_PREFIX}/prompts/${promptName}?version=${version.versionNumber}`,
                            {
                                headers: {
                                    'Content-Type': 'application/json',
                                    Authorization:
