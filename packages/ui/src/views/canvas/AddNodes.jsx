...
const AddNodes = forwardRef(({ nodesData, node, isAgentCanvas, isAgentflowv2, onFlowGenerated, hidden = false }, ref) => {
    ...
    useEffect(() => {
        if (prevOpen.current === true && open === false && !hidden && anchorRef.current && anchorRef.current.focus) {
            try {
                anchorRef.current.focus()
            } catch (error) {
                // Ignore focus errors when component is hidden
                console.warn('Focus error in AddNodes:', error)
            }
        }
